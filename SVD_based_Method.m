% Made by Woosung 2020.Feb.05th

function [F, index] = SVD_based_Method(H, K)
% < Input >
...
%% Default Value
if nargin < 1
    disp('[Message] SVD_Precoder_Selection : Default set H, K');
    nT = 2; nR = 4; 
    K = 2;
    H = 1/sqrt(K)*sqrt(1/2)*(randn(nR,nR)+1i*randn(nR,nR));
elseif nargin < 2
    disp('[Message] SVD_Precoder_Selection : Default set only K');
    K = 4;
end
...
%   curr_min = realmax;
    ...
    [~, S, ~] = svd(Hp);
    min_dist_per_precoder(curr_precoder_idx) = min(diag(S).^2);
