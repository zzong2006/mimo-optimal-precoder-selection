...
function [F, index] = QRD_based_BT_P_Method(H, K)
% < Input >
...
%% Default Value
if nargin < 1
    disp('[Message] QRD_BT-P_Precoder_Selection : Default set H, K');
    % When you try 4x2 MIMO env, should use nS (K) = 2 (2 streams)
    nT = 2; nR = 4; 
    K = 2;
    H = 1/sqrt(K)*sqrt(1/2)*(randn(nR, nR)+1i*randn(nR, nR));
elseif nargin < 2
    disp('[Message] QRD_BT-P_Precoder_Selection : Default set only K');
    K = 4;
end
...
for i = precoder_idx
    ...
    R_per_min = realmin;
    for perms_idx = perms_array      
        perm_Hp = Hp(:, perms_idx);
        ...
        [~, R] = qr(perm_Hp);
        R_per_min = max(R_per_min, min(diag(R).^2));
    end   
    min_dist_per_precoder(curr_precoder_idx) = R_per_min;
